...
The angular-size measurement depends on the model of the stellar intensity profile. SATLAS modifies the plane-parallel stellar atmosphere program ATLAS to treat spherically extended geometry. ATLAS calculates line blanketing using opacity distribution functions, which average the contributions of different atomic and molecular species. \citet{2018A&A...616A..68G} estimate the model error contribution to the angular size by considering intensity profiles at neighboring temperature, effective gravity, and stellar mass grid points.
PHOENIX is an alternative stellar atmosphere model that is inherently spherically symmetric and calculates opacities directly from a comprehensive library of 500,000 spectral lines \citep{2013A&A...553A...6H}.
\citet{2004A&A...413..711W} compared PHOENIX and tbe non-spherical ATLAS limb darkening models for the M4 giant star $\psi$~Phoenicis  in the $K$-band 
with the VLTI , finding to $\sim $1.5\% differences in
their resulting diameter values.  For the F5 star Procyon no differences are found in the $K$ band but
there is a significantly wide range of angular diameters at visible wavelengths (500~nm) in data from the Mark~III interferometer \citep{2005ApJ...633..424A}.
The PHOENIX intensity profile and squared visibility for the best-matched grid points are plotted with those of SATLAS in Figure~\ref{fig:Intensity} \citep{2023A&A...674A..63C}. By visual inspection, the only discernible difference is in the intensity at the outer edge of the stellar disk. The difference between the squared visibilities is $\lesssim 0.006$ over the range of the plot and $\lesssim 0.006$ over the spatial frequency range where the HD~360 data are fit. These differences are significantly smaller than the measurement error bars, demonstrating consistency between SATLAS and PHOENIX models for this application.
...
The technique does not require combining light coherently, eliminating systematic errors associated with beam combination optics, polarization mismatch, and the complex calibration chains needed to extract visibility amplitudes and closure phases. However, intensity interferometry introduces its own set of systematic challenges. The primary sources of systematic error include: clock synchronization uncertainties between the focal positions of widely separated telescopes, which directly affect the measured temporal correlations; baseline determination errors, as precise knowledge of telescope separations is essential for converting measured correlations to angular scales; the finite extent of the individual telescope mirrors,
which is not accounted for in the visibility calculation; 
background noise and accidental coincidences, which can bias correlation measurements, particularly for faint sources.
...
The measurement in intensity interferometry is the correlation between photon arrival times at two detector positions.  It is generated my making the normalized histogram of the difference in photon arrival times at the two telescopes.  The expected signal for
...
an overall throughput of 0.3, and an integration time of $T_{obs}=1$~h.
...
Interferometry with simultaneous observation of $N$ telescopes offers ${N \choose 2}$ unique baselines. For $N=37$ telescopes \citep[the number of Small-Sized Telescopes planned for the Cherenkov Telescope Array Observatory-South][]{2022SPIE12182E..0KT}, this yields an effective increase by a factor of 666 to the nominal exposure time. 
